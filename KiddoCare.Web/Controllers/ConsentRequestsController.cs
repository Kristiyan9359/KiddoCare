...
public class ConsentRequestsController : Controller
{
    ...
    public async Task<IActionResult> Index(string? searchTerm, string? statusFilter, string? returnUrl, int page = 1, int pageSize = 15)
    {
        ...
        var model = await consentRequestService.GetAllAsync(userId, isAdmin, isTeacher, searchTerm, statusFilter, page, pageSize);
    ...
    public async Task<IActionResult> Suggestions(string term)
    {
        ...
        var suggestions = await consentRequestService.GetSearchSuggestionsAsync(term, userId, isAdmin, isTeacher);
